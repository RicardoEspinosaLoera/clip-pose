...
class TripletDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        I_use  = cv2.resize(I_np,  (Hs, Hs), interpolation=cv2.INTER_LINEAR)
        BG_use = cv2.resize(BG_np, (Hs, Hs), interpolation=cv2.INTER_LINEAR)

        # Resize mask (nearest neighbor to preserve binary values)
        M_use  = cv2.resize(M_np,  (Hs, Hs), interpolation=cv2.INTER_NEAREST)
